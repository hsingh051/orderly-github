    ...
    <div class="row">
        <div class="col-sm-12 mt-5">
            <div class="settings Form">
                {!! Form::open(['url' => '/settings/store', 'method' => 'post']) !!}
                    <div class="form-group {{ $errors->has('authorize') ? ' has-error' : '' }}">
                        <h6>Gatway Setup</h6>
                        {!! Form::radio('gateway', 'authorize', true, ['class' => 'form-check-input']); !!}
                        {!! Form::label('gateway', 'Authorize.net', ['class' => 'form-check-label']); !!}
                    </div>
                    <div class="form-group {{ $errors->has('authorize_payment_api_login_id') ? ' has-error' : '' }}">
                        {!! Form::label('authorize_payment_api_login_id', 'Authorize.net Payment API Login ID*'); !!}
                        {!! Form::text('authorize_payment_api_login_id', null, ['class' => 'form-control']); !!}
                    </div>
                    <div class="form-group {{ $errors->has('authorize_payment_transaction_key') ? ' has-error' : '' }}">
                        {!! Form::label('authorize_payment_transaction_key', 'Authorize.net Payment Transaction Key*'); !!}
                        {!! Form::text('authorize_payment_transaction_key', null, ['class' => 'form-control']); !!}
                    </div>
                    {{-- <div class="form-group">
                        {!! Form::label('authorize_key', 'Authorize.net Key*'); !!}
                        {!! Form::text('authorize_key', null, ['class' => 'form-control']); !!}
                    </div> --}}
                    {!! Form::submit('Save Settings', ['class' => 'btn btn-primary mt-3']) !!}
                {!! Form::close() !!}
                </div>
            </div>
        </div>
